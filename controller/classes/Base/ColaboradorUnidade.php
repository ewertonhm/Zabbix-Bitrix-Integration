...
/**
 * Base class that represents a row from the 'colaborador_unidade' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class ColaboradorUnidade implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\ColaboradorUnidadeTableMap';
    ...
    /**
     * Initializes internal state of Base\ColaboradorUnidade object.
     */
    ...
    /**
     * Compares this with another <code>ColaboradorUnidade</code> instance.  If
     * <code>obj</code> is an instance of <code>ColaboradorUnidade</code>, delegates to
     * <code>equals(ColaboradorUnidade)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ColaboradorUnidadeTableMap::COL_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ColaboradorUnidadeTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ColaboradorUnidadeTableMap::translateFieldName('UnidadeId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->unidade_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ColaboradorUnidadeTableMap::translateFieldName('ColaboradorId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = ColaboradorUnidadeTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\ColaboradorUnidade'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildColaboradorUnidadeQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see ColaboradorUnidade::setDeleted()
     * @see ColaboradorUnidade::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildColaboradorUnidadeQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                ColaboradorUnidadeTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('colaborador_unidade_id_seq')");
        ...
        if ($this->isColumnModified(ColaboradorUnidadeTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(ColaboradorUnidadeTableMap::COL_UNIDADE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'unidade_id';
        }
        if ($this->isColumnModified(ColaboradorUnidadeTableMap::COL_COLABORADOR_ID)) {
            $modifiedColumns[':p' . $index++]  = 'colaborador_id';
        }

        $sql = sprintf(
            'INSERT INTO colaborador_unidade (%s) VALUES (%s)',
    ...
    {
        $pos = ColaboradorUnidadeTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['ColaboradorUnidade'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['ColaboradorUnidade'][$this->hashCode()] = true;
        $keys = ColaboradorUnidadeTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getUnidadeId(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\ColaboradorUnidade
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\ColaboradorUnidade The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ColaboradorUnidadeTableMap::COL_ID)) {
            $criteria->add(ColaboradorUnidadeTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ColaboradorUnidadeTableMap::COL_UNIDADE_ID)) {
            $criteria->add(ColaboradorUnidadeTableMap::COL_UNIDADE_ID, $this->unidade_id);
        }
        if ($this->isColumnModified(ColaboradorUnidadeTableMap::COL_COLABORADOR_ID)) {
            $criteria->add(ColaboradorUnidadeTableMap::COL_COLABORADOR_ID, $this->colaborador_id);
        }
    ...
    {
        $criteria = ChildColaboradorUnidadeQuery::create();
        $criteria->add(ColaboradorUnidadeTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ColaboradorUnidade (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ColaboradorUnidade Clone of current object.
     * @throws PropelException
     */
    ...
    {
        return (string) $this->exportTo(ColaboradorUnidadeTableMap::DEFAULT_STRING_FORMAT);
    }
