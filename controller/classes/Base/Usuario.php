...
abstract class Usuario implements ActiveRecordInterface
{
    ...
    public function setApiKey($v)
    {
        ...
        if ($this->api_key !== $v) {
            $this->api_key = $v;
            $this->modifiedColumns[UsuarioTableMap::COL_API_KEY] = true;
        }
    ...
    public function setBitrixId($v)
    {
        ...
        if ($this->bitrix_id !== $v) {
            $this->bitrix_id = $v;
            $this->modifiedColumns[UsuarioTableMap::COL_BITRIX_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UsuarioTableMap::translateFieldName('ApiKey', TableMap::TYPE_PHPNAME, $indexType)];
            $this->api_key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UsuarioTableMap::translateFieldName('BitrixId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->bitrix_id = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UsuarioTableMap::translateFieldName('Admin', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UsuarioTableMap::COL_API_KEY)) {
            $modifiedColumns[':p' . $index++]  = 'api_key';
        }
        if ($this->isColumnModified(UsuarioTableMap::COL_BITRIX_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UsuarioTableMap::COL_API_KEY)) {
            $criteria->add(UsuarioTableMap::COL_API_KEY, $this->api_key);
        }
        if ($this->isColumnModified(UsuarioTableMap::COL_BITRIX_ID)) {
            $criteria->add(UsuarioTableMap::COL_BITRIX_ID, $this->bitrix_id);
        }
        if ($this->isColumnModified(UsuarioTableMap::COL_ADMIN)) {
